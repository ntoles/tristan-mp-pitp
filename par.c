 ...
 *   1-mar-2004   Moved error out to utils.c as printf()         -- PJT
 ...
 *   int par_exist()       - returns 1 if block and name exists, 0 otherwise
...
void par_open(char *filename)
{
  ...
  if (now_open) printf("Parameter file %s still open\n",now_filename);
  if (now_filename) free(now_filename);
  fp = fopen(filename,"r");
  if (fp == NULL) 
    printf("Parameter file %s could not be opened, try -i PARFILE\n"
      ,filename);
...
void par_cmdline(int argc, char *argv[])
{
  ...
  for (i=1; i<argc; i++) {
    ...
    *ep = '\0';
    ...
    bp = find_block(block);
    if (bp == NULL) printf("par_cmdline: Block \"%s\" not found\n",block);
    pp = find_par(bp,name);
    if (pp == NULL) printf("par_cmdline: Par \"%s\" not found\n",name);
    free(pp->value);
...
int par_exist(char *block, char *name)
{
  ...
  if (!now_open) printf("par_exist: No open parameter file\n");
  if (block == NULL) printf("par_exist: no block name specified\n");
  if (name  == NULL) printf("par_exist: no par name specified\n");
  bp = find_block(block);
  if (bp == NULL) return 0;
  pp = find_par(bp,name);
  return (pp == NULL ? 0 : 1);
...
void par_dist_mpi(const int mytid, MPI_Comm comm)
{
  ...
  if(mytid == 0){    /* I'm the rank=0 process */
    if (!now_open) printf("Parameter file is not open\n");
    if(debug)
...
/* Share the filename with the children */
    ...
    if(MPI_SUCCESS != MPI_Bcast(&count, 1, MPI_INT, 0, comm))
      printf("[par_dist_mpi]: Error on calling MPI_Bcast\n");
...
/* Broadcast the "name = value # comment" string */
    ...
    if (now_open) printf("Parameter %s file is already open\n",now_filename);
...
static void *allocate(size_t size) 
{
  ...
  if(mem == NULL) printf("allocate: not enough memory for %d bytes\n",(int) size);
  return mem;
}
...
static char *my_strdup(char *in)
{
  ...
  if(out == NULL) printf("Error allocating memory for a new string\n");
  return out;
}
...
/* Skip leading white space and remember the start of block name */
  ...
  while (*cp != '\0' && *cp != '>')
    cp++;
  if (*cp != '>') printf("Blockname %s does not appear terminated\n",sp);
...
static void add_par_line(Block *bp, char *line)
{
  ...
  if(bp == NULL)
    printf("[add_par_line]: (no block name) while parsing line \n%s\n",line);
...
static char *par_getsl(char *block, char *name)
{
  ...
  if (!now_open) printf("par_gets: No open parameter file\n");
  if (block == NULL) printf("par_gets: no block name specified\n");
  if (name  == NULL) printf("par_gets: no par name specified\n");
  bp = find_block(block);
  if (bp == NULL) printf("par_gets: Block \"%s\" not found\n",block);
  pp = find_par(bp,name);
  if (pp == NULL)
    printf("par_gets: Par \"%s\" not found in Block \"%s\"\n",name,block);
  return pp->value;
}
...
/* int numprocs; */
  ...
  if(MPI_SUCCESS != MPI_Init(&argc,&argv))
    printf("Error on calling MPI_Init\n");
